...
<div class="wrapper-md">
	<div class="row">
		<div class="col-sm-8">
			<div class="blog-post">
				<div class="panel panel-post">
					...
					<div>
						<img src="{{ asset('uploaded/media/'.GlobalClass::setImages($post->image)) }}" width="100%">
					</div>
					<div class="wrapper-lg">
						...
						{{-- <div class="text-muted">
							...
							@if ($post->comment == 1 )
							@php
								$counts = DB::table('comments')->select('id_posts')->where('id_posts', $post->id)->count();
							@endphp
							...
							<div>
								<div>
									<a class="pull-left thumb-sm">
										<img src="{{ asset('uploaded/users') }}/{{ $c->image }}" class="img-circle">
									</a>
								...
								<div class="m-l-xxl">
									<a class="pull-left thumb-sm">
										<img src="{{ asset('uploaded/users/') }}/{{ $parent->image }}" class="img-circle">
									</a>
									<div class="m-l-xxl m-b">
										<div>
								...
								<div class="m-l-xxl">
									<form action="{{ route('comment_store') }}" method="post">
										...
										<a class="pull-left thumb-sm">
											<img src="{{ asset('uploaded/users').'/thumb-'.Auth::user()->image }}" class="img-circle">
										</a>
										<div class="m-l-xxl m-b">
		...
		<div class="col-sm-4">
			...
			<ul class="list-group">
				@foreach ($categories as $valuePosts)
			...
			<div>
				...
				<div>
					...
					<div class="img-recent" style="background-image:url('{{ asset('uploaded/media/'.GlobalClass::setImages($rp->image)) }}')">
					</div>
						<!-- <img src="{{ asset('uploaded/media/'.GlobalClass::setImages($rp->image)) }}" width="100%"> -->
					</a>
					<div class="clear">
						<a href="{{ route('posts_detail', ['slug' => $rp->slug ]) }}" class="font-semibold text-ellipsis">{{ $rp->title }}</a>
						<div class="text-xs block m-t-xs"><a href></a>{{ Carbon\Carbon::parse($rp->published)->diffForHumans() }}</div>
					</div>
				</div>
				<div class="line"></div>
				@endforeach
