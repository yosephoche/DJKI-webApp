		...
		<div class="col w-md bg-light dk b-r bg-auto">
			...
			<div class="wrapper hidden-sm hidden-xs" id="email-menu">
				<ul class="nav nav-pills nav-stacked nav-sm">
					...
					<li class="active"><a href="{{ route('messages_sent') }}">Sent</a></li>
				...
				<ul class="list-group list-group-lg no-radius m-b-none m-t-n-xxs">
					@forelse ($messages as $message)
						<li class="list-group-item clearfix {{ $message->read_status == '0' ? 'b-l-3x b-l-info' : '' }}">
							...
							<div class="clear">
								<div><a class="text-md {{ $message->read_status == '0' ? 'font-bold' : '' }}" href="{{ route('messages_detail_sent', ['id' => $message->id]) }}">{{ $message->email }}</a></div>
